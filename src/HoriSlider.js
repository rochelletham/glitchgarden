/**
 * @classdesc HoriSlider
 * A SVG-based horizontal slider web component
 */
export class HoriSlider extends HTMLElement {
  /**
   * Creates an instance of HoriSlider. Initializes component state, shadow DOM, and default values
   */
  constructor() {
    ...
    this._displayMult = 1;
    this._label = 'horislider';
    this._isDragging = false;
    // Used for vertical drag calculation
    this._initialPointerY = 0; 
    // Value when drag started
    this._initialValueOnDrag = 0; 
  ...
  static get observedAttributes() {
    return ['min-value', 'max-value', 'default-value', 'label', 'value', 'displayMult'];
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      case 'displayMult':
        this._displayMult = parseFloat(newValue) || 1;
      ...
      case 'default-value':
        ...
        break;
      case 'label':
        this._label = newValue ?? 'horislider';
        if (this.shadowRoot.querySelector('.horislider-label')) {
          this.shadowRoot.querySelector('.horislider-label').textContent =
            this._label;
        }
        break;
      case 'value':
  ...
  // Getters and Setters
  /**
   * Gets the current value of the horislider.
  ...
  /**
   * Sets the current value of the horislider.
  ...
  _setupInitialProperties() {
    // for displaying ex 0-1 to 0-100
    this._displayMult = parseFloat(this.getAttribute('displayMult')) || 1;
    ...
    if (this._maxValue <= this._minValue) {
      console.warn(
        `horisliderSimple: max-value must be greater than min-value. ` +
          `Adjusting max-value.`
      );
  ...
  /**
   * Converts the current horislider value to an angle for the tick mark.
  ...
  _render() {
    ...
    const radius = svgSize * 0.4; // horislider radius
    ...
    <style>
      :host {
        ...
        --horislider-bg-color: #4f565e;
        --horislider-fg-color: #8a9199;
        --horislider-tick-color: #00a1ff;
        --horislider-label-color: #d1d5db;
        --horislider-value-color: #d1d5db;
        --horislider-value-edit-bg: #fff;
      }
      .horislider-container {
        display: flex;
        flex-direction: column;
        align-items: center;
      ...
      .horislider-label {
        font-size: 14px;
        color: var(--horislider-label-color);
        margin-bottom: 5px;
        text-align: center;
        white-space: nowrap; /* Prevent label wrapping */
      }
      .horislider-svg-container {
      ...
      .horislider-circle {
        fill: var(--horislider-bg-color);
        stroke: var(--horislider-fg-color);
        stroke-width: 3;
      }
      .horislider-tick {
        stroke: var(--horislider-tick-color);
        stroke-width: 9; /* Make tick thicker */
        stroke-linecap: rounded; /* Rounded ends for the tick */
      }
      .horislider-value {
        font-size: 14px;
        color: var(--horislider-value-color);
        text-align: center;
      ...
      .horislider-value:focus {
        outline: none;
        background-color: var(--horislider-value-edit-bg);
        border: 1px solid var(--horislider-fg-color);
        box-shadow: 0 0 2px rgba(0,0,0,0.3);
      }
    </style>
    <div class="horislider-container" part="container">
      <div class="horislider-label" part="label">${this._label}</div>
      <div class="horislider-svg-container" part="svg-container">
    ...
    this._tickElement = this.shadowRoot.querySelector('.horislider-tick');
    this._valueElement = this.shadowRoot.querySelector('.horislider-value');
    this._labelElement = this.shadowRoot.querySelector('.horislider-label');
  ...
  _attachListeners() {
    ...
    const svgContainer = this.shadowRoot.querySelector('.horislider-svg-container');
  ...
  /**
   * Handles the pointerdown event on the horislider SVG area to initiate dragging.
  ...
  /**
   * Handles the pointermove event during dragging to update the horislider value.
   ...
   * Handles the click event on the horislider SVG area for Meta/Cmd+Click reset.
  ...
  _handleValueKeyDown(event) {
    if (event.key === 'Enter') {
      ...
      if (!isNaN(newValue)) {
        ...
        // converting the displayed value back to internal value for web audio
        const scaledValue = newValue / this._displayMult;
  ...
  /**
   * Updates the rotation of the horislider's tick mark based on the current value.
   * @private
   */
  _updateHoriSliderVisuals() {
  ...
  _updateValueDisplay() {
    ...
    if (this._valueElement.contentEditable !== 'true') {
      ...
      //   this._precision
      // );
      this._valueElement.textContent = Math.round(this._currentValue * this._displayMult);
    }
}
...
customElements.define('hori-slider', HoriSlider);